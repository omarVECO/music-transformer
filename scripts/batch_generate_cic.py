"""
Batch generation using the CIC checkpoint (best_model_cic.pt).
Produces results/cic_batch/<N>_<genre>_<mood>_<instrument>.mid

Usage (from project root):
    python scripts/batch_generate_cic.py [--max N]
"""
import argparse
import os
import subprocess
...
def main():
    ...
    out_dir = Path("results/cic_batch")
    ...
    for i, (midi_path, genre, mood, instrument) in enumerate(inputs, 1):
        ...
        print(f"\n[{i}/{len(inputs)}] {genre}/{mood}/{instrument}")
        cmd = [
            sys.executable, "scripts/generate_cic.py",
        ...
        result = subprocess.run(cmd, env={**os.environ}, capture_output=False)
